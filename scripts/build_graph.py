...
def build_edge_index_and_attr(
    ...
    GPU-vectorised: thresholding and feature gathering are done entirely on
    the device; no Python loop over channel pairs.
    ...
    """
    corr_mean = torch.from_numpy(
        edge_features["correlation"][:, :, 0].astype(np.float32)
    ).to(DEVICE)  # (C, C)
    n_channels = corr_mean.shape[0]
    n_feat = len(edge_feature_names)

    # Upper-triangle mask: pairs (i < j) with |corr| >= threshold
    mask = torch.triu(corr_mean.abs() >= threshold, diagonal=1)  # (C, C)
    src, dst = mask.nonzero(as_tuple=True)  # (n_edges,) each

    if src.numel() == 0:
        # Degenerate fallback: self-loops
        idx = torch.arange(n_channels, device=DEVICE)
        edge_index = torch.stack([idx, idx], dim=0)
        edge_attr = torch.zeros(n_channels, n_feat, device=DEVICE)
        return edge_index.cpu().numpy(), edge_attr.cpu().numpy()

    # Gather one scalar per edge feature (mean agg = index 0) at (src, dst)
    feat_cols = []
    for name in edge_feature_names:
        if name in edge_features:
            mat = torch.from_numpy(
                edge_features[name][:, :, 0].astype(np.float32)
            ).to(DEVICE)
            feat_cols.append(mat[src, dst])
        else:
            feat_cols.append(torch.zeros(src.shape[0], device=DEVICE))

    edge_attr_upper = torch.stack(feat_cols, dim=1)  # (n_edges, n_feat)
...
def zscore_features(features: np.ndarray) -> np.ndarray:
    """Z-score features along the node axis (axis 0) on GPU."""
    x = torch.from_numpy(features.astype(np.float32)).to(DEVICE)
    mean = x.mean(dim=0, keepdim=True)
    std = x.std(dim=0, keepdim=True)
    std[std < 1e-8] = 1.0
    return ((x - mean) / std).cpu().numpy()
